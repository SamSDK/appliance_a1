		...
		do {
			...
			case 1: {
				while (passwordAttempts <= 12) {
					if (passwordAttempts == 3 || passwordAttempts == 6 || passwordAttempts == 9
							|| passwordAttempts == 12) {
				...
				if (validAccess && menuChoice == 1) {
					...
					for (int i = 0; i < quantityOfObjects; i++) {
						if (inventory[i].getSerialNumber() == serialNumberAttempt) {
							...
							System.out.println(
									"Would you like to enter a new serial number? If not, program will terminate. (y/n)");
							String response = keyboard.nextLine();
							if (response.equalsIgnoreCase("y")) {
								System.out.println(
										" Reminder: this is your second attempt. What is the serial number of the item that you wish to update?:");
					...
					System.out.println(
							"Please restart program if you would like to continue. Password attempts have been exhausted");
				}
			}
			case 3: {
